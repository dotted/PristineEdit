...
namespace BadMechanics.PristineEdit.Common.Data
{
    ...
    public static class File
    {
        ...
        {
            string text;
            return DetectTextEncoding(filename, out text, taster);
        }

        public static Encoding DetectTextEncoding(string filename, out String text, int taster = 1000)
        {
            var file = FileSystem.Current.GetFileFromPathAsync(filename).Result;
            var open = file.OpenAsync(FileAccess.Read).Result;
            return DetectTextEncoding(open, out text, taster);
        }

        public static Encoding DetectTextEncoding(Stream stream, int taster = 1000)
        {
            string text;
            return DetectTextEncoding(stream, out text, taster);
        }
        ...
        /// 
        /// http://stackoverflow.com/a/12853721/1299957
        /// </summary>
        /// <param name="stream"></param>
        /// <param name="text"></param>
        /// <param name="taster"></param>
        /// <returns></returns>
        public static Encoding DetectTextEncoding(Stream stream, out String text, int taster = 1000)
        {
            var b = new byte[] { };
            ...
            else if (b.Length >= 4 && b[0] == 0xFF && b[1] == 0xFE && b[2] == 0x00 && b[3] == 0x00) { text = Encoding.GetEncoding("utf-32").GetString(b, 4, b.Length - 4); return Encoding.GetEncoding("utf-32"); }    // UTF-32, little-endian
            else if (b.Length >= 2 && b[0] == 0xFE && b[1] == 0xFF) { text = Encoding.BigEndianUnicode.GetString(b, 2, b.Length - 2); return Encoding.BigEndianUnicode; }     // UTF-16, big-endian
            else if (b.Length >= 2 && b[0] == 0xFF && b[1] == 0xFE) { text = Encoding.Unicode.GetString(b, 2, b.Length - 2); return Encoding.Unicode; }              // UTF-16, little-endian
            else if (b.Length >= 3 && b[0] == 0xEF && b[1] == 0xBB && b[2] == 0xBF) { text = Encoding.UTF8.GetString(b, 3, b.Length - 3); return Encoding.UTF8; } // UTF-8
            else if (b.Length >= 3 && b[0] == 0x2b && b[1] == 0x2f && b[2] == 0x76) { text = Encoding.GetEncoding("utf-7").GetString(b, 3, b.Length - 3); return Encoding.GetEncoding("utf-7"); } // UTF-7
            ...
            if (((double)count) / taster > Threshold) { text = Encoding.BigEndianUnicode.GetString(b, 0, b.Length); return Encoding.BigEndianUnicode; }
            count = 0;
            for (var n = 1; n < taster; n += 2) if (b[n] == 0) count++;
            if (((double)count) / taster > Threshold) { text = Encoding.Unicode.GetString(b, 0, b.Length); return Encoding.Unicode; } // (little-endian)
            ...
            for (var n = 0; n < taster - 9; n++)
            {
                        ...
                        && (b[n + 8] == '=')))
                {
                    ...
                    try
                    {
                        string internalEnc = Encoding.GetEncoding("us-ascii").GetString(nb, 0, nb.Length);
                        text = Encoding.GetEncoding(internalEnc).GetString(b, 0, b.Length);
            ...
            throw new NotSupportedException("Could not automatically detect file encoding");
        }
